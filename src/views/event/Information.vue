<template>
    ...
    <el-table :data="tableData" v-loading="tableDataLoading" style="width: 100%" height="550">
      <el-table-column fixed prop="type" label="类型" width="150">
        <template #default="scope">
          ...
          <el-tag v-if="scope.row.type === 'OTHER'" type="danger">其他</el-tag>
        </template>
      </el-table-column>
      <el-table-column fixed prop="title" label="标题" width="250"/>
      <el-table-column prop="releaseTime" label="发布时间" width="220"/>
      <el-table-column prop="resource" label="附件" width="150">
        <template #default="scope">
          <a :href="scope.row.resource" target="_blank" >下载</a>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="publishing" label="是否公开" width="120">
        <template #default="scope">
          <el-tag v-if="scope.row.publishing" type="success">是</el-tag>
          <el-tag v-if="!scope.row.publishing" type="danger">否</el-tag>
        </template>
      </el-table-column>
  ...
  <InformationEdit v-if="editVisible" :visible="editVisible" :row="data" :mode="mode" @onClose="(v) => closeEdit(v, state)" @onSave="(row) => handleEdit(row, state)"/>
  <!-- 组件 END -->
</template>
...
import { loadData, MODE,
  ...
  closeEdit,
  showEdit } from "./service/information.js";
